...
const typeFilters: TypeFilter[] = ["All Types", "Maintenance Request", "Password Reset Request", "Submission Delay", "Threshold Breach", "System Health"];
...
export function AdminAlertsMonitorPage() {
  ...
  const handleStatusChange = (alert: PriorityAlert, status: PriorityAlertStatus) => {
    updateAlertStatus(alert.id, status, authUser?.displayName ?? "Admin");
  };

  return (
    <PageMotion>
      <PageHeader title="Alerts" description="Centralized alert feed for IT requests, enterprise issues, reporting delays, threshold breaches, and system health events." />
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-240 table-fixed text-left text-sm">
            ...
            <thead className="bg-gray-50 text-[10px] font-bold tracking-wider text-gray-500 uppercase">
              <tr>
                {["Alert ID", "Type", "Priority", "Source", "Required Action", "Status", "Actions"].map((heading) => (
